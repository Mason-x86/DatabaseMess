...
class CheckinMember:
    ...
    def bridge_checkin_request(self, check_in_date):
        try:
            select_stmt = (
                "SELECT Check_ID FROM check_In "
            ...
            insert_stmt = (
                "INSERT INTO Employee_Check_In_Bridge (Check_ID, Employ_ID)"
                "VALUES (%s, %s)"
            )
    ...
    def checkin_request(self):
        ...
        try:
            ...
            insert_stmt = (
                "INSERT INTO check_In(check_In_date, check_in_time)"
                "VALUES (%s, %s)"
            )
            ...
            self.bridge_checkin_request(date_stamped)
    ...
    def __init__(self, master):
        ...
        self.date_label = Label(master, text="check in time (yr:mt:dy)", padx=40)
        self.time_label = Label(master, text="check in time (hr:mn)", padx=40)
        self.add_button = Button(master, text="Add", padx=20, command=lambda: self.checkin_request())
